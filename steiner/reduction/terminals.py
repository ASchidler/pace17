            ...
            for t in list(steiner.terminals):
                ...
                if min_node in steiner.terminals or len(neighbors) == 1:
                    ...
                    if len(neighbors) == 1:
                        ...
                        change = True
                    # The closest node is a terminal? The edge will be in an optimal solution, therefore merge the nodes
                    elif min_node in steiner.terminals:
                        self._removed.append((t, min_node, min_val))
                        for ng in neighbors:
                            if ng != min_node:
                                d = steiner.graph[t][ng]['weight']
                                if steiner.add_edge(min_val, ng, d + min_val):
                                    self._selected.append(((min_val, ng, d + min_val), (t, ng, d)))
